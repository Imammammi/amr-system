        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs mt-6">
            <div class="flex items-center justify-between mb-4">
                <h4 class="text-lg font-semibold text-gray-600 dark:text-gray-300">
                    Device templates ({{ number_format(count($device_templates)) }})
                </h4>
                <div class="flex space-x-2">
                                ...
                                <div class="w-full">
                                    ...
                                    <form wire:submit.prevent="saveDevice_template">
                                            ...
                                            <div class="mt-2">
                                                <label class="text-sm">
                                                    <input wire:model.defer="title" type="text"
                                                        ...
                                                        placeholder="Device_template title">
                                                    <x-input-error for="title"></x-input-error>
                                                </label>
                                            </div>

                                            <!-- Write Device_template -->
                                            <div class="mt-2">
                                                <label class="text-sm">
                                                    <textarea wire:model.defer="body"
                                                        ...
                                                        rows="7" placeholder="Device_template content"></textarea>
                                        ...
                                        <footer
                                            ...
                                            <button wire:loading.remove wire:click="saveDevice_template"
                                            ...
                                            <button wire:loading wire:target="saveDevice_template"
                    ...
                    <a href="{{ url('admin/device_templates-trashed') }}">
                ...
                <table id="html5-table" class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3">Device template</th>
                            <th class="px-4 py-3">Wait time</th>
                            <th class="px-4 py-3">Resend Count</th>
                        ...
                        @foreach ($device_templates as $device_template)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3 text-sm">
                                    <a href="{{ url('admin/device_template') }}/{{ $device_template->slug }}/{{ $device_template->id }}">
                                        {{ $device_template->name }}
                                    </a>
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $device_template->wait_time }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $device_template->resend_count }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    Tukur Hamid
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $device_template->created_at->format('d-m-Y') ?? '' }}
                                </td>
                                        ...
                                        <div x-cloak x-show="open"
                                            ...
                                            <div x-show="open" x-transition:enter="transition ease-out duration-150"
                                                ...
                                                <div class="w-full pb-6">
                                                    <!-- Modal description -->
                                                    <form wire:submit.prevent="deleteDevice_template({{ $device_template->id }})">
                                                        <div class="bg-white rounded-lg dark:bg-transparent pt-6">
                                                            <h3 class="text-lg">Trash
                                                                <span class="font-bold">{{ $device_template->title }}</span>?
                                                            </h3>
                                                        </div>
                                                    </form>
                                                </div>
                                                ...
                                                <footer
                                                    ...
                                                    <button wire:loading wire:target="deleteDevice_template({{ $device_template->id }})"
                                                    ...
                                                    <button wire:loading.remove
                                                        wire:click="deleteDevice_template({{ $device_template->id }})"
                                                        class="w-full py-2 px-5 text-sm text-white bg-red-600 hover:bg-red-700 cursor-pointer font-medium leading-5 transition-colors duration-150 border rounded-lg sm:w-auto sm:px-4 sm:py-2 focus:outline-none focus:shadow-outline-blue">
                                                        Yes
                                                    </button>
                                                </footer>
